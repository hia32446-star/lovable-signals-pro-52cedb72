 ...
 export const ChartView = () => {
     ...
     try {
       ...
       if (data && data.candles.length > 0) {
         ...
         setDetectedPatterns(analysis.patterns);
          ...
          setApiOnline(false);
          setDetectedPatterns([]);
          toast.error('Live market candles are unavailable for this pair');
        }
      } catch (error) {
        ...
        setApiOnline(false);
        setDetectedPatterns([]);
        toast.error('Failed to fetch live market candles');
      } finally {
       setIsLoading(false);
     }
   }, [selectedPair]);
   ...
   const handleDownloadChart = async () => {
     try {
       ...
       URL.revokeObjectURL(url);
        ...
        toast.success('Chart saved with live market data');
      } catch (error) {
        console.error('Failed to generate chart:', error);
        toast.error('Failed to generate live chart image');
      }
   };
   ...
   // Calculate moving averages
    const { ma20, ma50, support, resistance } = useMemo(() => {
      if (candles.length === 0) {
        return { ma20: [], ma50: [], support: 0, resistance: 0 };
      }
   ...
   // Price range
    const allPrices = candles.flatMap(c => [c.high, c.low]);
    const minPrice = allPrices.length > 0 ? Math.min(...allPrices) - 0.0005 : 0;
    const maxPrice = allPrices.length > 0 ? Math.max(...allPrices) + 0.0005 : 1;
    const priceRange = Math.max(maxPrice - minPrice, 0.00001);
  
    // Scale functions
    const xScale = (i: number) => padding.left + (i / Math.max(candles.length - 1, 1)) * plotWidth;
    const yScale = (price: number) => padding.top + ((maxPrice - price) / priceRange) * plotHeight;
  
    const candleWidth = Math.max(4, (plotWidth / Math.max(candles.length, 1)) * 0.7);
       ...
       <div className="glass-panel p-4">
          <div className="text-center mb-4">
            ...
            <p className="text-xs text-muted-foreground">
              {isLiveData ? '🔴 Live Data' : '⚠️ Live Data Unavailable'} • {candles.length} candles
            </p>
          </div>
         
          {candles.length === 0 ? (
            <div className="flex h-[450px] items-center justify-center rounded-lg border border-border bg-card/40 text-sm text-muted-foreground">
              Live market candle data is unavailable right now.
            </div>
          ) : (
